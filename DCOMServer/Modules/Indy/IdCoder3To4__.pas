//SDA 09-10.09.2002
// Исправил TIdBase64Decoder.CompleteCoding.
// В некоторых закодированных блоках не верно обрабатывался символ '='.
...
uses
  IdGlobal__, IdMIMETypes__, IdResourceStrings__,
  SysUtils;
...
begin
  ...
  sOut:='';
  tmpPS:=0;
  for tmpI:=1 to FCBufferedData do begin
    tmpByte:=Ord(s1[tmpI]);
    if tmpByte=Ord('=') Then break;
    tmpByte:=GetTableIndex(Chr(tmpByte));
    Inc(tmpPS, 2);
    tmpCurr.asWord:=0;
    tmpCurr.asWord:=tmpByte shl tmpPS;
    Case tmpPS of
      2:begin{2:00000000'XXXXXX00}
        tmpLast:=tmpCurr;
      end;
      4,6,8:begin{4:000000XX'XXXX0000} {6:0000XXXX'XX000000} {8:00XXXXXX'00000000}
        tmpLast.asLoByte:=tmpLast.asLoByte Or tmpCurr.asHiByte;
        sOut:=sOut+Chr(tmpLast.asLoByte);
        tmpLast:=tmpCurr;
        If tmpPS=8 then tmpPS:=0;
      end;
    end;
  end;
