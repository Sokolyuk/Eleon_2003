...
type
  ...
  TIdAttachment__ = class(TIdMessagePart)
  protected
...
procedure TIdAttachment__.Assign(Source: TPersistent);
var
  mp: TIdAttachment__;
begin
  if ClassType <> Source.ClassType then
...
constructor TIdAttachment__.Create(Collection: TIdMessageParts; const AFileName:
  TFileName = '');
begin
...
function TIdAttachment__.GetContentDisposition: string;
begin
  Result := Headers.Values['Content-Disposition']; {do not localize}
end;

function TIdAttachment__.SaveToFile(const FileName: TFileName): Boolean;
begin
  Result := CopyFileTo(StoredPathname, FileName);
end;

procedure TIdAttachment__.SetContentDisposition(const Value: string);
begin
...
initialization
  //RegisterClasses([TIdAttachment__, TIdText]);
end.
