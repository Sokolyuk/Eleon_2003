// Corrected by SDA 28.05.01
// Corrected by SDA 10.08.01
// Corrected by SDA 25.10.01
...
resourcestring
  sTextNotSet = 'SQL строка не определена.';
  sIllegalSpecialChar = 'Illegal special character.';
  sIllegalStringChar = 'Illegal string delimiter.';

type
  TTokenType = (ttString, ttSymbol, ttComment, ttDelimiter, ttSpecialChar, ttStatementDelimiter, ttCommentedSymbol, ttCommentDelimiter);
  TSetOfChar = set of Char;
  TComment = array[0..1] of string;
  TCharacterType = (ctSymbol, ctBeginComment, ctEndComment, ctDelimiter, ctString, ctSpecialChar);
...
//  SQLParserComp_DataCase:IDataCase=Nil;

type
  TEndOfStatement = procedure(Sender: TObject; SQLStatement: String) of object;

  TQueryParserComp=class(TObject)//(TComponent)
  private
  ...
  published
    property IsEOFStmtDelimiter: Boolean read FGenerateOnStmtDelimiter write FGenerateOnStmtDelimiter;
    property StringDelimiters: String read FStringDelimiters write SetStringDelimiters;
    property SpecialCharacters: String read FSpecialCharacters write SetSpecialCharacters;
    property RemoveStrDelimiter: Boolean read FRemoveStrDelimiter write FRemoveStrDelimiter;
    property CountFromStatement: Boolean read FCountFromStatement write FCountFromStatement;
    property TextToParse: String read FStringToParse write SetStringToParse;
    property StatementDelimiters: TStringList read FStatementDelimiters write SetSD;
    property OnStatementDelimiter: TEndOfStatement read FOnStatementDelimiter write FOnStatementDelimiter;
  end;
...
begin
  ...
  FreeAndNil(FStatementDelimiters);
  inherited Destroy;
end;
...
begin
  ...
  begin
    ...
    if FStream.Position>0 then{SDA}FStream.Seek(-1, soFromCurrent) else begin
{      If Assigned(SQLParserComp_DataCase) Then begin
        SQLParserComp_DataCase.ITMessAdd(Now, Now, 'debug', Self, '-------', 'TQueryParserComp', '(1) To ignore Seec(-1). Position='+IntToStr(FStream.Position)+', Size='+IntToStr(FStream.Size)+'.', mecDebug, mesWarning);
      end;}
    end;
  end;
end;
...
begin
  ...
  if not FString then FStringDelimiter:=' ';
  FToken:='';
  if not FEOF then begin
    ...
    if Length(Buffer)>0 then begin
      if (Buffer[1] in Delimiters) and not(FString or FComment) then
        ...
        if FStream.Position>0 then FStream.Seek(-1, soFromCurrent);
        SetToken;
      end;
    end else FEOF:=True;
  end;
  case FTokenType of
    ...
    ttString:begin
      ...
      if FRemoveStrDelimiter then RemoveStringDelimiter(FToken);
    end;
  end;
  if StatementDelimiter then FTokenType:=ttStatementDelimiter;
  if FEOF then begin
    FLastStringDelimiterFound:='';
    FWasString:=False;
    FString:=False;
    if FGenerateOnStmtDelimiter then DoStatementDelimiter;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if FStream.Position>0 then{SDA} FStream.Seek(-1, soFromCurrent) else begin
{        If Assigned(SQLParserComp_DataCase) Then begin
          SQLParserComp_DataCase.ITMessAdd(Now, Now, 'debug', Self, '-------', 'TQueryParserComp', '(2) To ignore Seec(-1). Position='+IntToStr(FStream.Position)+', Size='+IntToStr(FStream.Size)+'.', mecDebug, mesWarning);
        end;}
      end;
    end;
    ...
    begin
      ...
      begin
        ...
        begin
          if FStream.Position>0 then{SDA}FStream.Seek(-1, soFromCurrent) else begin
{            If Assigned(SQLParserComp_DataCase) Then begin
              SQLParserComp_DataCase.ITMessAdd(Now, Now, 'debug', Self, '-------', 'TQueryParserComp', '(3) To ignore Seec(-1). Position='+IntToStr(FStream.Position)+', Size='+IntToStr(FStream.Size)+'.', mecDebug, mesWarning);
            end;}
          end;
          FTokenType := ttSymbol;
        end
    ...
    begin
      ...
      begin
        ...
        begin
          if FStream.Position>0 then{SDA}FStream.Seek(-1, soFromCurrent) else begin
{            If Assigned(SQLParserComp_DataCase) Then begin
              SQLParserComp_DataCase.ITMessAdd(Now, Now, 'debug', Self, '-------', 'TQueryParserComp', '(4) To ignore Seec(-1). Position='+IntToStr(FStream.Position)+', Size='+IntToStr(FStream.Size)+'.', mecDebug, mesWarning);
            end;}
          end;
    ...
    ctSpecialChar:begin
      if FToken<>'' then begin
        if FStream.Position>0 then{SDA}FStream.Seek(-1, soFromCurrent) else begin
{          If Assigned(SQLParserComp_DataCase) Then begin
            SQLParserComp_DataCase.ITMessAdd(Now, Now, 'debug', Self, '-------', 'TQueryParserComp', '(5) To ignore Seec(-1). Position='+IntToStr(FStream.Position)+', Size='+IntToStr(FStream.Size)+'.', mecDebug, mesWarning);
          end;}
        end;
        FTokenType := ttSymbol;
      end else begin
  ...
  for i := 1 to Length(FStringDelimiters) do
    ...
    begin
      ...
      begin
        if not FEOF then
          Buffer := FStream.ReadString(1);
//          FEOF := FStream.Position >= FStream.Size-1; {SDA 2001}
        ...
        begin
          ...
          begin
            if FStream.Position>0 then{SDA}FStream.Seek(-1, soFromCurrent) else begin
{              If Assigned(SQLParserComp_DataCase) Then begin
                SQLParserComp_DataCase.ITMessAdd(Now, Now, 'debug', Self, '-------', 'TQueryParserComp', '(6) To ignore Seec(-1). Position='+IntToStr(FStream.Position)+', Size='+IntToStr(FStream.Size)+'.', mecDebug, mesWarning);
              end;}
            end;
            Result := True;
          end
...
begin
  ...
  begin
    ...
    Buffer := FStream.ReadString(1);
    if Not(Length(Buffer)>0) then begin
      FEOF := True;
      Continue;
    end;
    ...
    begin
      ...
      begin
        ...
        if FStream.Position>0 then{SDA}FStream.Seek(-1, soFromCurrent) else begin
{          If Assigned(SQLParserComp_DataCase) Then begin
            SQLParserComp_DataCase.ITMessAdd(Now, Now, 'debug', Self, '-------', 'TQueryParserComp', '(7) To ignore Seec(-1). Position='+IntToStr(FStream.Position)+', Size='+IntToStr(FStream.Size)+'.', mecDebug, mesWarning);
          end;}
        end;
      end
      else
      begin
        ...
        begin
          ...
          begin
            ...
            if FStream.Position>0 then{SDA}FStream.Seek(-1, soFromCurrent) else begin
{              If Assigned(SQLParserComp_DataCase) Then begin
                SQLParserComp_DataCase.ITMessAdd(Now, Now, 'debug', Self, '-------', 'TQueryParserComp', '(8) To ignore Seec(-1). Position='+IntToStr(FStream.Position)+', Size='+IntToStr(FStream.Size)+'.', mecDebug, mesWarning);
              end;}
            end;
            FComment := False;
            FCommentType := [];
  ...
  If FEOF Then begin
    If (FStream.Position >= FStream.Size)=False Then
      FEOF := False;
  End;
end;
...
begin
  ...
  begin
    ...
    if FStream.Size-1>=FGoPosition then{SDA}FStream.Seek(FGoPosition, soFromBeginning) else begin
{      If Assigned(SQLParserComp_DataCase) Then begin
        SQLParserComp_DataCase.ITMessAdd(Now, Now, 'debug', Self, '-------', 'TQueryParserComp', '(-1) To ignore Seec('+IntToStr(FGoPosition)+'). Position='+IntToStr(FStream.Position)+', Size='+IntToStr(FStream.Size)+'.', mecDebug, mesWarning);
      end;}
    end;
    Buffer:=FStream.ReadString(CurrentPosition-FGoPosition-Length(FToken));
    if (FStream.Size-1)>=(Length(FToken)+FStream.Position) then{SDA}FStream.Seek(Length(FToken), soFromCurrent) else begin
{      If Assigned(SQLParserComp_DataCase) Then begin
        SQLParserComp_DataCase.ITMessAdd(Now, Now, 'debug', Self, '-------', 'TQueryParserComp', '(-2) To ignore Seec('+IntToStr(FGoPosition)+'). Position='+IntToStr(FStream.Position)+', Size='+IntToStr(FStream.Size)+'.', mecDebug, mesWarning);
      end;}
    end;
